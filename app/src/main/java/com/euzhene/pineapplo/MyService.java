...
public class MyService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        Toast.makeText(getApplicationContext(), "Alarm will set in " + MainActivity.timerViewModel.GetTotalTimeInMinutes() + " minutes", Toast.LENGTH_SHORT).show();
        return START_NOT_STICKY;
    }

    private void createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            NotificationChannel notificationChannel = new NotificationChannel(CHANNEL_ID, "pinepplo running in foreground", NotificationManager.IMPORTANCE_HIGH);
